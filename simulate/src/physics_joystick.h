...
class PS3Joystick : public unitree::common::UnitreeJoystick
{
public:
    PS3Joystick(std::string device, int bits = 15)
...
// BeTop USB joystick
class BeTopJoystick : public unitree::common::UnitreeJoystick
{
public:
    BeTopJoystick(std::string device, int bits = 15)
	: unitree::common::UnitreeJoystick()
	{
		js_ = std::make_unique<Joystick>(device);
		if(!js_->isFound()) {
    ...
    {
        js_->getState();
        back(js_->button_[6]); // select button
        start(js_->button_[7]);
        LB(js_->button_[4]);
        RB(js_->button_[5]);
        A(js_->button_[0]);
